	...
	Для Сч = 1 По Число(ЧислоПопыток) Цикл
		...
		Попытка
			...
			Если Сеансы.Количество() Тогда
				...
				ОтключитьСоединенияСРабочимиПроцессами(V8, Агент, Кластер, ИмяБазы, ИмяАдминистратораИБ, ПарольАдминистратораИБ, ИмяАдминистратораКластера, ПарольАдминистратораКластера);
			КонецЕсли;
...
Функция ПолучитьСписокСеансов(Агент, ПортСервера, ИмяБазы, ИмяАдминистратораИБ, ПарольАдминистратораИБ, ИмяАдминистратораКластера, ПарольАдминистратораКластера)
	...
	Попытка		
		...
		Для Каждого Кластер из Кластеры Цикл
			...
			//ТекущиеСеансы.Вставить("ИнформационнаяБаза", ИнформационнаяБаза);
...
Процедура ОтключитьСоединенияСРабочимиПроцессами(V8, Агент, Кластер, ИмяБазы, ИмяАдминистратораИБ, ПарольАдминистратораИБ, ИмяАдминистратораКластера, ПарольАдминистратораКластера)
	...
	Попытка		
		...
		Для Каждого РабочийПроцесс Из Процессы Цикл
			Если РабочийПроцесс.Running И РабочийПроцесс.IsEnable  Тогда
				...
				CоединениеСРабочимПроцессом = V8.ConnectWorkingProcess(СтрокаСоединенияСРабочимПроцессом);

				CоединениеСРабочимПроцессом.AddAuthentication(ИмяАдминистратораИБ, ПарольАдминистратораИБ);

				Шаблон 	 		= CоединениеСРабочимПроцессом.CreateInfoBaseInfo();
		Шаблон.Name 	= ИмяБазы;
				СоединенияБазы 	= CоединениеСРабочимПроцессом.GetInfoBaseConnections(Шаблон); 
		...
		Для Каждого Соединение Из СоединенияБазы Цикл
			...
			//	Продолжить;
			//КонецЕсли;
					CоединениеСРабочимПроцессом.Disconnect(Соединение);

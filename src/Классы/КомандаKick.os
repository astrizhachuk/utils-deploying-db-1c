...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	Если НЕ ПустаяСтрока(ПутьКФайлуНастроек) Тогда
		...
		Если ФайлНастроек.Существует() Тогда
			...
			Если НЕ ЗначениеЗаполнено(АдминистраторКластера) Тогда
				АдминистраторКластера = Параметры.НастройкиПоУмолчанию.АдминистраторКластера;
			КонецЕсли;

			Если НЕ ЗначениеЗаполнено(ПарольАдминистратораКластера) Тогда
				ПарольАдминистратораКластера = Параметры.НастройкиПоУмолчанию.ПарольАдминистратораКластера;
			КонецЕсли;

			Если НЕ ЗначениеЗаполнено(ИнформационнаяБаза) Тогда
				ИнформационнаяБаза = Параметры.НастройкиПоУмолчанию.ИнформационнаяБаза;
			КонецЕсли;

			Лог.Отладка("Обработана настройка <%1>, установлен параметр <%2>", "АдминистраторКластера", АдминистраторКластера);
			Лог.Отладка("Обработана настройка <%1>, установлен параметр <%2>", "ПарольАдминистратораКластера", ПарольАдминистратораКластера);
			Лог.Отладка("Обработана настройка <%1>, установлен параметр <%2>", "ИнформационнаяБаза", ИнформационнаяБаза);
			Лог.Отладка("Обработана настройка <%1>, установлен параметр <%2>", "КоличествоПопыток", КоличествоПопыток);
		Иначе
